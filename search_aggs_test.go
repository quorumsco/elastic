...
func TestAggs(t *testing.T) {
	...
	if esversion >= "1.4" {
		countByUserAgg := NewFiltersAggregation().Filters(NewTermFilter("user", "quorumsco"), NewTermFilter("user", "sandrae"))
	...
	if termsAggRes.Buckets[0].Key != "quorumsco" {
		t.Errorf("expected %q; got: %q", "quorumsco", termsAggRes.Buckets[0].Key)
	}
...
// does not yield base64 encoded data. See https://github.com/quorumsco/elastic/issues/51
...
func TestAggsTerms(t *testing.T) {
	...
	if agg.Buckets[0].Key != "quorumsco" {
		t.Errorf("expected key %q; got: %q", "quorumsco", agg.Buckets[0].Key)
	}
...
func TestAggsSubAggregates(t *testing.T) {
	...
	if users.Buckets[0].Key != "quorumsco" {
		t.Errorf("expected key %q; got: %q", "quorumsco", users.Buckets[0].Key)
	}
...
/*
// TestAggsRawMessage is a test for issue #51 (https://github.com/quorumsco/elastic/issues/51).
