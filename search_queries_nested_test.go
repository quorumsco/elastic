...
func TestNestedQueryWithInnerHit(t *testing.T) {
	...
	f = f.Query(bq)
	f = f.QueryName("qname")
	f = f.InnerHit(NewInnerHit().Name("comments").Query(NewTermQuery("user", "quorumsco")))
	data, err := json.Marshal(f.Source())
	if err != nil {
		t.Fatalf("marshaling to JSON failed: %v", err)
	}
	got := string(data)
	expected := `{"nested":{"_name":"qname","inner_hits":{"name":"comments","query":{"term":{"user":"quorumsco"}}},"path":"obj1","query":{"bool":{"must":[{"term":{"obj1.name":"blue"}},{"range":{"obj1.count":{"from":5,"include_lower":false,"include_upper":true,"to":null}}}]}}}}`
	if got != expected {
		t.Errorf("expected\n%s\n,got:\n%s", expected, got)
	}
}
