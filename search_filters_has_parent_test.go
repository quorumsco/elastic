...
func TestHasParentFilterWithInnerHitsQuery(t *testing.T) {
	f := NewHasParentFilter("blog")
	f = f.Query(NewTermQuery("tag", "something"))
	f = f.InnerHit(NewInnerHit().Query(NewTermQuery("user", "quorumsco")))
	data, err := json.Marshal(f.Source())
	if err != nil {
		t.Fatalf("marshaling to JSON failed: %v", err)
	}
	got := string(data)
	expected := `{"has_parent":{"inner_hits":{"query":{"term":{"user":"quorumsco"}}},"parent_type":"blog","query":{"term":{"tag":"something"}}}}`
	if got != expected {
		t.Errorf("expected\n%s\n,got:\n%s", expected, got)
	}
}
