...
func TestNestedFilterWithInnerHit(t *testing.T) {
	...
	got := string(data)
	expected := `{"nested":{"_cache":true,"inner_hits":{"name":"comments","query":{"term":{"user":"quorumsco"}}},"path":"obj1","query":{"bool":{"must":[{"term":{"obj1.name":"blue"}},{"range":{"obj1.count":{"from":5,"include_lower":false,"include_upper":true,"to":null}}}]}}}}`
	if got != expected {
		t.Errorf("expected\n%s\n,got:\n%s", expected, got)
	}
}
